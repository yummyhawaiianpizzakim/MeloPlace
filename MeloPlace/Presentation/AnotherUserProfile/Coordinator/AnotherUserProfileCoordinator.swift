//
//  AnotherUserProfileCoordinator.swift
//  MeloPlace
//
...
final class AnotherUserProfileCoordinator: CoordinatorProtocol {
    var finishDelegate: CoordinatorFinishDelegate?
    
    var childCoordinators: [CoordinatorProtocol] = []
    
    var type: CoordinatorType = .anotherUserProfile
    ...
    func start() {
        self.showAnotherUserProfileViewFlow()
    }
    
    private func showAnotherUserProfileViewFlow() {
        let container = DIContainer.shared.container
        guard let vm = container.resolve(AnotherUserProfileViewModel.self) else { return }
        
        let vc = AnotherUserProfileViewController(viewModel: vm)
        
        vm.userID = self.userID
        vm.setActions(
            actions: AnotherUserProfileViewModelActions(
...
extension AnotherUserProfileCoordinator: CoordinatorFinishDelegate {
